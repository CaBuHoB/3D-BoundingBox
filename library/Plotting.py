...
def create_2d_box(box_2d):
    ...
    val_pt1 = corner1_2d
    val_pt2 = (corner1_2d[0], corner2_2d[1])
    val_pt3 = corner2_2d
    val_pt4 = (corner2_2d[0], corner1_2d[1])

    return val_pt1, val_pt2, val_pt3, val_pt4
...
def plot_3d_pts(\
    img, pts, center, calib_file=None, cam_to_img=None, relative=False, constraint_idx=None):
    """ plot 3d pts """
    if calib_file is not None:
        cam_to_img = get_calibration_cam_to_image(calib_file)
    ...
    for it_pt in pts:
        if relative:
            it_pt = [i + center[j] for j, i in enumerate(it_pt)] # more pythonic

        point = project_3d_pt(it_pt, cam_to_img)
