...
def rotation_matrix(yaw):
    ...
    val_ty = yaw
    val_ry = np.array(\
        [[np.cos(val_ty), 0, np.sin(val_ty)], [0, 1, 0], \
            [-np.sin(val_ty), 0, np.cos(val_ty)]])
...
def calc_location(dimension, proj_matrix, box_2d, alpha, theta_ray):
    ...
    for constraint in constraints:
        ...
        val_ma = np.copy(pre_m)
        val_mb = np.copy(pre_m)
        val_mc = np.copy(pre_m)
        val_md = np.copy(pre_m)

        m_array = [val_ma, val_mb, val_mc, val_md]

        # create A, b
        val_big_a = np.zeros([4, 3], dtype=np.float)
        val_b = np.zeros([4, 1])
        ...
        for row, index in enumerate(indicies):
            ...
            val_rx = np.dot(val_r, val_x)
            val_m[:3, 3] = val_rx.reshape(3)

            val_m = np.dot(proj_matrix, val_m)

            val_big_a[row, :] = val_m[index, :3] - box_corners[row] * val_m[2, :3]
            val_b[row] = box_corners[row] * val_m[2, 3] - val_m[index, 3]

        # solve here with least squares, since over fit will get some error
        loc, error, _, _ = np.linalg.lstsq(val_big_a, val_b, rcond=None)
