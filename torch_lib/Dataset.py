...
class Dataset(data.Dataset):
    ...
    def __init__(self, path, calib_path, bins=2, overlap=0.1):
        ...
        for obj in self.object_list:
            cur_id = obj[0]
            line_num = obj[1]
            label = self.get_label(cur_id, line_num)
            if cur_id != last_id:
                self.labels[cur_id] = {}
                last_id = cur_id

            self.labels[cur_id][str(line_num)] = label
    ...
    def get_label(self, val_id, line_num):
        """ Gets label """
        lines = open(self.top_label_path + '%s.txt'%val_id).read().splitlines()
    ...
    def get_bin(self, angle):
        ...
        def is_between(val_min, val_max, angle):
            ...
            angle = (angle - val_min) if (angle - val_min) > 0 else (angle - val_min) + 2*np.pi
    ...
    def format_label(self, line):
        ...
        val_box_2d = [top_left, bottom_right]

        val_dim = np.array([line[8], line[9], line[10]], dtype=np.double) # height, width, length
        # modify for the average
        val_dim -= self.averages.get_item(val_class)

        val_loc = [line[11], line[12], line[13]] # x, y, z
        val_loc[1] -= val_dim[0] / 2 # bring the KITTI center up to the middle of the object

        val_orient = np.zeros((self.bins, 2))
        val_conf = np.zeros(self.bins)
        ...
        for bin_idx in bin_idxs:
            ...
            val_orient[bin_idx, :] = np.array([np.cos(angle_diff), np.sin(angle_diff)])
            val_conf[bin_idx] = 1
    ...
    def parse_label(self, label_path):
        ...
        with open(label_path, 'r') as file:
            for line in file:
                ...
                val_dim = [line[8], line[9], line[10]] # height, width, length
                val_loc = [line[11], line[12], line[13]] # x, y, z
                val_loc[1] -= val_dim[0] / 2 # bring the KITTI center up to the middle of the object
    ...
    def all_objects(self):
        ...
        for it_id in self.ids:
            ...
            for label in labels:
                ...
                objects.append(\
                    DetectedObject(img, detection_class, box_2d, proj_matrix, label=label))
